...
local Window = Rayfield:CreateWindow({
   ...
   Discord = {
      ...
      Invite = "https://discord.gg/CCVbX59EAv", -- The Discord invite code, do not include discord.gg/. E.g. discord.gg/ ABCD would be ABCD
...
-- Prevent character from dying instantly from physics glitches
...
local Paragraph = Tab:CreateParagraph({Title = "Disclaimer", Content = "Speed, Jump Power, Godmode, and No fall damage is glitchy, dont go to high on the speed or jump power or you will die!"})
...
local Button = Tab:CreateButton({
 ...
 local Toggle = Tab:CreateToggle({
    Name = "God Mode",  -- Name of the toggle button
    CurrentValue = false,  -- Default value (off)
    Flag = "GodModeToggle",  -- Identifier for configuration saving
    Callback = function(Value)
        local player = game.Players.LocalPlayer
        local humanoid = player.Character and player.Character:FindFirstChild("Humanoid")
        
        if humanoid then
            if Value then  -- If toggle is on, enable God Mode
                -- Set the Health to a very high value to prevent dying
                humanoid.MaxHealth = math.huge  -- Set max health to infinity
                humanoid.Health = humanoid.Health  -- Retain current health value
                humanoid.HealthChanged:Connect(function()  -- Prevent health from decreasing
                    if humanoid.Health < humanoid.MaxHealth then
                        humanoid.Health = humanoid.MaxHealth  -- Keep health full
                    end
                end)
            else  -- If toggle is off, disable God Mode
                humanoid.MaxHealth = 100  -- Set max health back to normal (default)
                if humanoid.Health < humanoid.MaxHealth then
                    humanoid.Health = humanoid.MaxHealth  -- Restore health to default
                end
            end
        end
    end,
 ...
 local Toggle = Tab:CreateToggle({
    Name = "No Fall Damage",  -- Name of the toggle button
    CurrentValue = false,  -- Default value (off)
    Flag = "NoFallDamageToggle",  -- Identifier for configuration saving
    Callback = function(Value)
        local player = game.Players.LocalPlayer
        local humanoid = player.Character and player.Character:FindFirstChild("Humanoid")
        
        if humanoid then
            if Value then  -- If toggle is on, disable fall damage
                humanoid:SetAttribute("NoFallDamage", true)  -- Use a custom attribute to track fall damage
                humanoid:GetPropertyChangedSignal("FloorMaterial"):Connect(function()
                    if humanoid:GetAttribute("NoFallDamage") then
                        humanoid.Health = humanoid.Health  -- Prevent any damage
                    end
                end)
            else  -- If toggle is off, enable fall damage
                humanoid:SetAttribute("NoFallDamage", false)  -- Remove custom attribute
            end
        end
    end,
 })
...
local Tab = Window:CreateTab("TP's", 4483362458) -- Title, Image
...
local Button = Tab:CreateButton({
    ...
    Callback = function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(-239, 179, 330))
        end
    end,
})

-- Game TP Button
local GameButton = Tab:CreateButton({
    Name = "Game",
    Callback = function()
        local char = game.Players.LocalPlayer.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(-28, 47, 75))
        end
    end,
})
